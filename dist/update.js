...
const update = async (shouldCommit = false) => {
    ...
    const [owner, repo] = (0, secrets_2.getOwnerRepo)();
...
`,
                            ...
                            try {
                                const downmsg = await (0, secrets_1.getSecret)("NOTIFICATIONS_DOWN_MESSAGE") ? ((0, secrets_1.getSecret)("NOTIFICATIONS_DOWN_MESSAGE") || "")
                                    .replace("$SITE_NAME", site.name)
                                    .replace("$SITE_URL", `(${site.url})`)
                        ...
                        try {
                            const upmsg = await (0, secrets_1.getSecret)("NOTIFICATIONS_UP_MESSAGE") ? ((0, secrets_1.getSecret)("NOTIFICATIONS_UP_MESSAGE") || "")
                                .replace("$SITE_NAME", site.name)
                                .replace("$SITE_URL", `(${site.url})`)
                                : `$EMOJI ${site.name} (${site.url}) $STATUS`;
                            await (0, notifme_1.sendNotification)(upmsg.replace("$EMOJI", `${config.commitPrefixStatusUp || "🟩"}`).replace("$STATUS", `${issues.data[0].title.includes("degraded")
                                ? "performance has improved"
                                : "is back up"}`));
                        }
                        catch (error) {
    ...
    if (restartInstances.length) {
        ...
        for (const domain of restartInstances) {
            ...
            if (ec2InstanceKey && region) {
                const ec2InstanceId = (0, secrets_1.getSecret)(ec2InstanceKey) || '';
                aws_sdk_1.default.config.update({
                    accessKeyId: (0, secrets_1.getSecret)("ACCESS_KEY_ID"),
                    secretAccessKey: (0, secrets_1.getSecret)("SECRET_ACCESS_KEY"),
                    region: (0, secrets_1.getSecret)(region),
                });
                console.log(`instance domain ${domain}`);
                console.log(`instance key ${ec2InstanceKey}`);
                console.log(`instance region key ${region}`);
                console.log(`instance region ${(0, secrets_1.getSecret)(region)}`);
